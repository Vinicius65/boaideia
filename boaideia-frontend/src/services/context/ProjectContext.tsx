...
export function ProjectProvider({ children }: { children: any }) {
    const [_projectList, _setProjectList] = useState<TProject[]>(ProjectListMock.map(p => {
        return {
            ...p,
            all: `${p.name} ${p.description} ${p.goalList.map(g => `${g.name} ${g.description}`)} ${p.userList.map(u => `${u.firstname} ${u.lastname}`)}`
        }
    }))
    const [projectList, setProjectList] = useState<TProject[]>([]);

    const filter = (query: string) => setProjectList(!query ? _projectList : _projectList.filter(p => p.all.includes(query)));
